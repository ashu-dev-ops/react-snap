...
const pagesThatHaveTitle=[]
const pagesThatDontHaveTitle=[]
...
const saveAsHtml = async ({ page, filePath, options, route, fs }) => {
  ...
  try {
    ...
    if (titleTagMatch[1]) {
      console.log("The page contains a <title> tag.");
      pagesThatHaveTitle.push({
        title: titleTagMatch[1],
        route: route,
      });
    } else {
      console.log("The page does not contain a <title> tag.");
      pagesThatDontHaveTitle.push({
        route,
      });
    }
  } catch (error) {
...
const run = async (userOptions, { fs } = { fs: nativeFs }) => {
  ...
  await crawl({
    ...
    onEnd: () => {
      ...
      console.log('pages that have title',pagesThatHaveTitle.length,pagesThatHaveTitle)
      console.log("pages that dont have title",pagesThatDontHaveTitle.length,pagesThatDontHaveTitle)
      if (http2PushManifest) {
